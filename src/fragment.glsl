...
void main() {
	...
	vec4 color1 = texelColor; //mix(vec4(vec3(1.0), 1.0), texelColor, easeOutCubic(min(noise / 0.2, 1.0)));
	vec4 color2 = vec4(vec3(219.0 / 255.0, 225.0 / 255.0, 255.0 / 255.0), logoColor.a);
            //gl_FragColor = vec4(mix(color1.rgb, color2.rgb, 1.0 - color1.a), max(color1.a, color2.a));
            //gl_FragColor = vec4(easeInCubic(max(0.0, (1.0 - fullUV.y) - (1.0 - fadeOut)) / fadeOut), 0.0, 0.0, 1.0);
